...
def main(args):
    ...
    for dset in args.eval_datasets:
        ...
        dataset = Dataset.from_file(dset)
        text_column_name = get_text_column_names(dataset.column_names)
        dataset = dataset.cast_column("audio", Audio(sampling_rate=16000))
        dataset = dataset.map(normalise, num_proc=16)
        ...
        for ref, hyp in zip(references, predictions):
            result_file.write('REF: ' + ref.encode('utf-8').decode('ascii', 'ignore') + '\n')
            result_file.write('HYP: ' + hyp.encode('utf-8').decode('ascii', 'ignore') + '\n')
            result_file.write("------------------------------------------------------" + '\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default="/home/ubuntu/uzair/fine_tuning_stt/output_model_dir/checkpoint-500",
